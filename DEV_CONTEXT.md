...
## 11. 시간 소요 섹션 제거 및 조교/근로학생 업무 매뉴얼 탭 정식 개설 (2026-09-04 업데이트)

1. **소요 시간(약 X분 소요) 배지 일괄 제거**:
   - `ManualStepCard.tsx`에서 시간 소요 인디케이터를 완전히 제거하여 화면을 더욱 심플하고 직관적으로 정돈.
2. **조교/근로학생 업무 매뉴얼 (`src/data/staffManual.ts`, `src/components/StaffManualView.tsx`) 정식 구현**:
   - 상단 세 번째 탭 클릭 시 즉시 렌더링되는 전용 매뉴얼 화면 구축.
   - **근무 시간표 카드**: 근로학생(학기 중 9~17:30 / 방학 중 10~17:00) vs 조교(9~20:00 마감 소등).
   - **4대 기본 수칙**: 근무 태도(인사, 시간 엄수), 일정 조정(최소 1주일 전 사전 연락), 방문객 응대("어떤 일로 오셨나요?"), 부재중 전화 메모 요령.
   - **공간 미화 및 환경 정비**: 오전 출근 시, 출근 직후 1층 공통 관리, 20시 퇴근 시 소등/장비실 잠금 점검.
   - **공간 사용 절차 안내**: 3D프린터실(K-MOOC 4대 기초교육 + 공간대여신청서 + 연구원 문의), VR실(신청서 + 1주 연장), 컨퍼런스홀/오픈형강의장/무한상상실(담당자 문의).
   - 자체 **PDF/인쇄 출력** 지원.

## 12. 다각도 다중 분할 사진 시스템 구축 (2026-09-04 업데이트)

1. **동일 항목 다중 사진 네이밍 규격 지원**:
   - 기본/단일 사진: `OpeningManual_{id}.jpg` (혼자 있을 시 1장 전체 화면 노출)
   - 서브 추가 사진: `OpeningManual_{id}_{sub}.jpg` (예: `OpeningManual_2_1.jpg`, `OpeningManual_2_1_2.jpg` 등)
   - 정렬 엔진: 기본 사진(`_` 접미사 없음)이 항상 1순위(왼쪽), 이후 서브 번호에 따른 자연 정렬(Natural Sort) 적용.
2. **동적 화면 분할 레이아웃 (`ManualStepCard.tsx`)**:
   - **1장**: 기존대로 꽉 찬 단독 프레임 (`aspect-[4/3] sm:aspect-[16/10]`)
   - **2장**: 양옆 2분할 뷰 (`grid-cols-2`, 좌: 기본, 우: 세부)
   - **3장**: 3중 분할 뷰 (`grid-cols-3`, 좌 → 우 순서대로 정렬)
   - **4장 이상**: 다분할 반응형 그리드 뷰 (`grid-cols-2 sm:grid-cols-4`)
3. **고도화된 라이트박스 뷰어 (`ImageLightboxModal.tsx`)**:
